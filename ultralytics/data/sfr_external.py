...
def prepare_aitodv2_from_raw(raw_root: Path, output_root: Path, *, yaml_path: Path, image_mode: str = "symlink") -> None:
    ...
    train_images = find_first_existing(raw_root, ["train/images", "images/train", "aitod/images/train"])
    val_images = find_first_existing(raw_root, ["val/images", "images/val", "aitod/images/val"])
    train_json = find_first_existing(
        raw_root,
        [
            "train.json",
    ...
    if not all([train_images, val_images, train_json, val_json]):
        raise FileNotFoundError(
            "AI-TOD-v2 raw root is incomplete. Expected a layout like raw/train/images + raw/train.json + "
            "raw/val/images + raw/val.json, or an equivalent aitod/images/{train,val} + annotations/{train,val}.json layout."
        )
